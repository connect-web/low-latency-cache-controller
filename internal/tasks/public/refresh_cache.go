...
func compareTimestamps(host string) (finished bool) {
	...
	dbSkillToplist, err := database.SkillToplist()
	if err != nil {
		fmt.Printf("Failed to fetch skill toplist: %s\n", err.Error())
		return false
	}

	dbMinigameToplist, err := database.MinigameToplist()
	if err != nil {
	...
	if invalidSkillRows.HasOutdatedEntries() {
		skillUrls := invalidSkillRows.BuildUrls("/api/v2/public/skill-toplist-users?skill-id=%d")
		requests.RefreshCache(host, skillUrls)
		requests.RefreshCache(host, []string{"/api/v2/public/skill-toplist"})
	}

	if invalidMinigameRows.HasOutdatedEntries() {
		minigameUrls := invalidMinigameRows.BuildUrls("/api/v2/public/boss-minigame-toplist-users?minigame-id=%d")
		requests.RefreshCache(host, minigameUrls)
		requests.RefreshCache(host, []string{"/api/v2/public/boss-minigame-toplist"})
	}
